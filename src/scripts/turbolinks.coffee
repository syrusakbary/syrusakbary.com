...
fetch = (url) ->
  current = new ComponentUrl
  url = new ComponentUrl url

  return if current.withoutHash() is url.withoutHash() and not (url.withoutHash() is url.href)
...
fetchHistory = (cachedPage, is_back=false) ->
  xhr?.abort()
  changePage cachedPage.title, cachedPage.body, null, null, null, is_back
...
cacheCurrentPage = ->
  ...
  # console.log 'CACHE PAGE', currentState.url, $(document.body).html()
...
changePage = (title, body, csrfToken, runScripts, onReplace, is_back) ->
  doFinish = ->
  ...
  if currentState.url is document.location.href
    document.documentElement.replaceChild body, document.body
    doFinish()
    return
...
reflectNewUrl = (url) ->
  if (url = new ComponentUrl url).absolute isnt referer
    window.history.pushState { turbolinks: true, url: url.absolute, order: historyItems++ }, '', url.absolute
...
rememberCurrentUrl = ->
  window.history.replaceState { turbolinks: true, url: document.location.href, order: historyItems++ }, '', document.location.href
...
class ComponentUrl
  ...
  withoutHash: -> 
    @href.replace new RegExp("(#|#{@hash})\s*$"), ''
...
class Link extends ComponentUrl
  ...
  _anchored: ->
    p = ((@withoutHash()) is (current = new ComponentUrl).withoutHash()) and
      not (@href is @withoutHash())

    # ((@hash and @withoutHash()) is (current = new ComponentUrl).withoutHash()) or 
    #   (@href.replace(/\#.*/,"#") is current.href.replace(/\#.*/,"#"))
...
installHistoryChangeHandler = (event) ->
  if event.state?.turbolinks
    if cachedPage = pageCache[(new ComponentUrl(event.state.url)).absolute]
      is_back = currentState.order > event.state.order
      # console.log 'BACK', back
      cacheCurrentPage()
      fetchHistory cachedPage, is_back
    else
...
@Turbolinks = { visit, pagesCached, enableTransitionCache, changePageAnimation, allowLinkExtensions: Link.allowExtensions, supported: browserSupportsTurbolinks }